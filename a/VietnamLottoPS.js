...
function parseHtml(result){
   ...
   var addSql='insert ignore into VietnamLottoPS(link,number1,number2,number3,number4,number5,number6,number7,number8,number9,number10,number11,number12,number13,number14,number15,number16,number17,number18) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)'; 
  connection.query(addSql,params,function(err,resul){
      if(err){
        console.log('[VietnamLottoPS insert error]-',err.message);
      return;
      }
    console.log('[VietnamLottoPS insert OK]');
});
connection.end(); 
};
});};
exports.VietnamLottoPS=VietnamLottoPS;
