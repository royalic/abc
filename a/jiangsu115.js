...
function jiangsu115(){var connection=mysql.createConnection({
 ...
 database:'lottery'});
connection.connect();
http.get('https://kjh.55128.cn/history_js11x5.aspx', function(req, res) {
    var result = '';
...
var addSql='insert ignore into jiangsu115(link,firstnum,secondnum,thirdnum,forthnum,fifthnum) values (?,?,?,?,?,?)';
...
connection.query(addSql,addSqlParams,function(err,resul){
  if(err){
  console.log('[jiangsu115 insert error]-',err.message);
  return;
}
console.log('jiangsu115 insert id:',resul);
});
});connection.end();};
};
